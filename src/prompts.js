...
SUMMARY: [2–3 sentences relating the quotes to the user's situation. Avoid markdown, be empathetic, and provide plain-text spiritual insight.]`
  ...
  userPrompts: {
    bhagavadGita: `IMPORTANT: Provide at least 5 relevant quotes ONLY from the Bhagavad Gita. For each quote, you MUST provide QUOTE, SOURCE, and CONTEXT. Do NOT include quotes from any other texts (e.g., Quran, Bible, Guru Granth Sahib, Vedas). Focus on Hindu spiritual wisdom directly from the Gita. Each quote must follow the strict format and be connected to the user's situation. TRANSLATE ALL QUOTES INTO THE USER'S LANGUAGE - do not keep quotes in English when user writes in another language. ALL FIELDS (QUOTE, SOURCE, CONTEXT, SUMMARY) ARE MANDATORY. DO NOT OMIT ANY FIELD.`,

    vedas: `IMPORTANT: Provide at least 5 relevant quotes ONLY from The Vedas. For each quote, you MUST provide QUOTE, SOURCE, and CONTEXT. Do NOT include quotes from any other texts (e.g., Bhagavad Gita, Quran, Bible, Guru Granth Sahib). Focus on Hindu spiritual wisdom directly from The Vedas. Each quote must follow the strict format and be connected to the user's situation. TRANSLATE ALL QUOTES INTO THE USER'S LANGUAGE - do not keep quotes in English when user writes in another language. ALL FIELDS (QUOTE, SOURCE, CONTEXT, SUMMARY) ARE MANDATORY. DO NOT OMIT ANY FIELD.`,

    quran: `IMPORTANT: Provide at least 5 relevant quotes ONLY from the Quran. For each quote, you MUST provide QUOTE, SOURCE, and CONTEXT. Do NOT include quotes from other texts (e.g., Bhagavad Gita, Bible, etc.). For Quran citations, use only SURAH NUMBERS — for example: "Quran 2:155" (no Surah names). Focus entirely on Islamic teachings. TRANSLATE ALL QUOTES INTO THE USER'S LANGUAGE - do not keep quotes in English when user writes in another language. ALL FIELDS (QUOTE, SOURCE, CONTEXT, SUMMARY) ARE MANDATORY. DO NOT OMIT ANY FIELD.`,

    bible: `IMPORTANT: Provide at least 5 relevant quotes ONLY from the Bible. For each quote, you MUST provide QUOTE, SOURCE, and CONTEXT. Do NOT include quotes from any other religious texts. When citing verses, use the standard book names (e.g., Matthew, Psalms, Genesis), and omit the word "Bible". Follow the exact quote format for each. TRANSLATE ALL QUOTES INTO THE USER'S LANGUAGE - do not keep quotes in English when user writes in another language. ALL FIELDS (QUOTE, SOURCE, CONTEXT, SUMMARY) ARE MANDATORY. DO NOT OMIT ANY FIELD.`,

    guruGranthSahib: `IMPORTANT: Provide at least 5 relevant quotes ONLY from the Guru Granth Sahib. For each quote, you MUST provide QUOTE, SOURCE, and CONTEXT. Do NOT use quotes from any other scriptures. Use Ang (page) number as the chapter, and the line number as the verse (e.g., Guru Granth Sahib 123:45). Focus on Sikh teachings and wisdom. TRANSLATE ALL QUOTES INTO THE USER'S LANGUAGE - do not keep quotes in English when user writes in another language. ALL FIELDS (QUOTE, SOURCE, CONTEXT, SUMMARY) ARE MANDATORY. DO NOT OMIT ANY FIELD.`,

    allTexts: `IMPORTANT: Provide spiritual guidance using at least 5 quotes TOTAL, with at least one quote from EACH of the following: The Bhagavad Gita, The Vedas, The Quran, The Bible, and The Guru Granth Sahib. Use a round-robin approach to ensure each tradition is represented. For each quote, you MUST provide QUOTE, SOURCE, and CONTEXT. Do NOT include non-scriptural quotes. Always follow the exact quote format. TRANSLATE ALL QUOTES INTO THE USER'S LANGUAGE - do not keep quotes in English when user writes in another language. ALL FIELDS (QUOTE, SOURCE, CONTEXT, SUMMARY) ARE MANDATORY. DO NOT OMIT ANY FIELD.`
  },
