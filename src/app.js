window.addEventListener('DOMContentLoaded', () => {
    ...
    function waitForConfig(timeout = 2000) {
        return new Promise((resolve, reject) => {
            const start = Date.now();
            (function check() {
                if (window.API_CONFIG) return resolve(window.API_CONFIG);
                if (Date.now() - start > timeout) return reject(new Error('API_CONFIG not available'));
                setTimeout(check, 50);
            })();
        });
    }

    waitForConfig().then(API_CONFIG => {
        ...
        alert('Could not load API configuration. Please refresh or contact support.');
    });
});
