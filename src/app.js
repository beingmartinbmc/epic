async function waitForApiConfig(maxRetries = 10, delay = 50) {
    ...
    throw new Error('❌ API_CONFIG not available after waiting');
}

window.addEventListener('DOMContentLoaded', async () => {
    try {
        const API_CONFIG = await waitForApiConfig();
        ...
        console.error('❌ Failed to load app due to config error:', err);
        const errorMessage = document.getElementById('errorMessage');
        if (errorMessage) {
            errorMessage.textContent = 'Configuration failed. Please reload.';
            errorMessage.style.display = 'block';
        }
    }
});
